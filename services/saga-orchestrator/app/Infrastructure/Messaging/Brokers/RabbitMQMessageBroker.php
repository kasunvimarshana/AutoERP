...
class RabbitMQMessageBroker implements MessageBrokerInterface
{
    ...
    public function publish(string $topic, array $message, array $options = []): bool
    {
        try {
            ...
            $exchange = $options['exchange'] ?? 'saga.events';
    ...
    {
        if ($message instanceof AMQPMessage) { $message->ack(); }
    }

    public function reject(mixed $message, bool $requeue = false): void
    {
        if ($message instanceof AMQPMessage) { $message->nack($requeue); }
    }

    public function isConnected(): bool { return $this->connection?->isConnected() ?? false; }
