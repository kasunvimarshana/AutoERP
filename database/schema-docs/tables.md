...
| `vehicle_ownerships` | Stores vehicle ownership records used by the owning module. | `customer_id` -> `customers`; `tenant_id` -> `tenants`; `organization_unit_id` -> `organization_units`; `vehicle_id` -> `vehicles` | tenant scoped; organization-unit aware |
...
| `vehicles` | Stores vehicle master, identity numbers, classifications, and ownership context. | `vehicle_make_id` -> `vehicle_makes`; `vehicle_type_id` -> `vehicle_types`; `organization_unit_id` -> `organization_units`; `vehicle_category_id` -> `vehicle_categories`; `vehicle_model_id` -> `vehicle_models`; `customer_id` -> `customers`; `tenant_id` -> `tenants` | unique `tenant_id,chassis_number`; unique `tenant_id,code`; unique `tenant_id,engine_number`; unique `tenant_id,vehicle_number`; unique `tenant_id,registration_number`; unique `tenant_id,vin_number`; tenant scoped; organization-unit aware; soft deletes |
