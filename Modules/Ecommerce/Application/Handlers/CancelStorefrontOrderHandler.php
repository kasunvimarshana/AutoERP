...
class CancelStorefrontOrderHandler extends BaseHandler
{
    ...
    public function handle(CancelStorefrontOrderCommand $command): StorefrontOrder
    {
        return $this->transaction(function () use ($command): StorefrontOrder {
            return $this->pipeline
                ...
                ->then(function (CancelStorefrontOrderCommand $cmd): StorefrontOrder {
                    ...
                    $cancellableStatuses = [
                        StorefrontOrderStatus::Pending->value,
                        StorefrontOrderStatus::Confirmed->value,
                    ];

                    if (! in_array($order->status, $cancellableStatuses, true)) {
                        throw new \DomainException("Order cannot be cancelled in its current status: '{$order->status}'.");
                    }
                    ...
                    return $this->repository->save(new StorefrontOrder(
                        ...
                        status: StorefrontOrderStatus::Cancelled->value,
