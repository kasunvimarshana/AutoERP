...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('finance_journal_entries', function (Blueprint $table): void {
            ...
            $table->char('source_identity_key', 64)->nullable();
            $table->char('source_fingerprint', 64)->nullable();
            $table->string('source_number', 150)->nullable();
            $table->date('source_date')->nullable();
            $table->string('journal_type', 30)->default('general');
            $table->string('status', 30)->default('draft');
            $table->text('description')->nullable();
            $table->decimal('total_debit', 20, 6)->default('0.000000');
            $table->decimal('total_credit', 20, 6)->default('0.000000');
            $table->foreignId('currency_id')->nullable()->constrained('currencies', 'id', indexName: 'finance_journal_entries_currency_fk')->restrictOnDelete();
            ...
            $table->unique(['tenant_id', 'source_identity_key'], 'finance_journals_source_identity_uk');
            ...
            $table->foreign(['fiscal_year_id', 'tenant_id'], 'finance_journal_entries_fiscal_year_id_tenant_fk')
                ->references(['id', 'tenant_id'])->on('finance_fiscal_years')->restrictOnDelete();
            $table->foreign(['fiscal_period_id', 'tenant_id'], 'finance_journal_entries_fiscal_period_id_tenant_fk')
                ->references(['id', 'tenant_id'])->on('finance_fiscal_periods')->restrictOnDelete();
            $table->foreign(['posting_profile_id', 'tenant_id'], 'finance_journal_entries_posting_profile_id_tenant_fk')
                ->references(['id', 'tenant_id'])->on('finance_posting_profiles')->restrictOnDelete();
