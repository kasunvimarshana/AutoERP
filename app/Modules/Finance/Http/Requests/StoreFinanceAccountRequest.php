...
final class StoreFinanceAccountRequest extends TenantScopedRequest
{
    public function rules(): array
    {
        ...
        return [
            ...
            'code' => ['required', 'string', 'max:100', $this->tenantUnique('finance_accounts', 'code')->ignore(is_numeric($accountId) ? (int) $accountId : null)],
    ...
    public function toData(): CreateAccountData
    {
        return new CreateAccountData(
            tenantId: $this->tenantId(), accountTypeId: (int) $this->input('account_type_id'),
            code: (string) $this->input('code'), name: (string) $this->input('name'),
            ...
            isControlAccount: $this->boolean('is_control_account'), isPostingAccount: $this->boolean('is_posting_account', true),
            isCashAccount: $this->boolean('is_cash_account'), isBankAccount: $this->boolean('is_bank_account'),
            isTaxAccount: $this->boolean('is_tax_account'), isActive: $this->boolean('is_active', true), metadata: $this->input('metadata'),
        );
    }
}
