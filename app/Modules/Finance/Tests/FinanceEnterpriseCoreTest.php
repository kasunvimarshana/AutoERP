...
final class FinanceEnterpriseCoreTest extends TestCase
{
    ...
    public function test_currency_revaluation_posts_through_profile_mappings(): void
    {
        ...
        $this->withTenantExecutionContext($tenantId, function () use ($tenantId, $bank, $gain, $loss): void {
    ...
    public function test_budget_actuals_are_calculated_from_ledger(): void
    {
        [$tenantId, $bank, $capital] = $this->context();
        $this->withTenantExecutionContext($tenantId, function () use ($tenantId, $bank, $capital): void {
            $profile = $this->profile($tenantId, 'budget_actual', [
                'bank' => $bank,
                'capital' => $capital,
            ]);
    ...
    private function profile(int $tenantId, string $code, array $rules): FinancePostingProfile
    {
        return $this->withTenantExecutionContext($tenantId, function () use ($tenantId, $code, $rules): FinancePostingProfile {
            ...
            foreach ($rules as $key => $account) {
                $roleId = $this->accountRole($tenantId, (string) $key, Str::headline((string) $key));
