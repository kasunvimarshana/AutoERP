...
final class InventoryReservationAllocationTest extends InventoryTestCase
{
    ...
    public function test_release_reservation_restores_availability(): void
    {
        [$tenantId, $warehouseId, $item] = $this->stockContext();
        $this->receipt($tenantId, $warehouseId, $item, '10.000000', '5.000000');
        $reservation = $this->withTenantExecutionContext($tenantId, fn () => app(StockReservationService::class)->reserve(new ReservationData($tenantId, '2026-06-06', (int) $item->getKey(), $warehouseId, '3.000000')));

        $this->withTenantExecutionContext($tenantId, fn () => app(StockReservationService::class)->release($reservation));
        ...
        $this->assertSame(ReservationStatus::Released, $this->withTenantExecutionContext($tenantId, fn () => $reservation->refresh()->status));
    }
