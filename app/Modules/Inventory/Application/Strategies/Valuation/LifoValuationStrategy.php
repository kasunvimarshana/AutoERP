...
final class LifoValuationStrategy implements ValuationStrategyInterface
{
    public function calculate(ValuationRequest $request): ValuationResult
    {
        ...
        if ($direction === StockDirection::IN) {
            ...
            if ($unitCost <= 0) {
                throw new InvalidArgumentException('Unit cost is required for inbound LIFO valuation.');
            }
        ...
        if ($remaining > 0) {
            throw new InvalidArgumentException('Insufficient inventory layers for LIFO valuation.');
        }
