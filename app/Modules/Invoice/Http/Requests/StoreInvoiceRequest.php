...
final class StoreInvoiceRequest extends TenantScopedRequest
{
    ...
    public function toData(): CreateInvoiceData
    {
        return new CreateInvoiceData(
            ...
            lines: array_map(static fn (array $row): InvoiceLineData => new InvoiceLineData(
            ...
            sources: array_map(fn (array $row): InvoiceSourceData => new InvoiceSourceData(
                tenantId: $this->tenantId(),
            ...
            sourceLines: array_map(fn (array $row): InvoiceSourceLineData => new InvoiceSourceLineData(
                tenantId: $this->tenantId(),
                sourceType: (string) $row['source_type'],
                sourceId: (int) $row['source_id'],
                ...
                remainingQuantity: isset($row['remaining_quantity']) ? (string) $row['remaining_quantity'] : null,
                invoicedLineTotal: isset($row['invoiced_line_total']) ? (string) $row['invoiced_line_total'] : null,
            ), $this->input('source_lines', [])),
            adjustments: array_map(static fn (array $row): InvoiceAdjustmentData => new InvoiceAdjustmentData(
