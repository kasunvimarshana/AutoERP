...
$middleware = [
    ...
    'tenant.permission:invoices.view',
];

Route::prefix('api/v1/invoices')->middleware($middleware)->name('api.v1.invoices.')->group(function (): void {
    Route::get('/', [InvoiceController::class, 'index'])->name('index');
    Route::post('preview', [InvoiceController::class, 'preview'])->middleware('tenant.permission:invoices.create')->name('preview');
    ...
    Route::post('{invoice}/approve', [InvoiceController::class, 'approve'])->whereNumber('invoice')->middleware('tenant.permission:invoices.approve')->name('approve');
    Route::post('{invoice}/post', [InvoiceController::class, 'post'])->whereNumber('invoice')->middleware('tenant.permission:invoices.post')->name('post');
    Route::post('{invoice}/cancel', [InvoiceController::class, 'cancel'])->whereNumber('invoice')->middleware('tenant.permission:invoices.cancel')->name('cancel');
