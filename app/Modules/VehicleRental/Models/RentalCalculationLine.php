...
final class RentalCalculationLine extends TenantOwnedModel
{
    ...
    {
        static::deleting(static function (RentalCalculationLine $line): void {
            $status = $line->status instanceof RentalCalculationLineStatus
                ? $line->status
                : RentalCalculationLineStatus::from((string) $line->status);

            if ($status !== RentalCalculationLineStatus::Draft) {
                throw new LogicException('Only draft rental calculation lines can be deleted. Use calculation reversal for billing history.');
            }
        });
    }
