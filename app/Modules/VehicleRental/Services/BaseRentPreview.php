...
final class BaseRentPreview
{
    ...
    public function calculate(AgreementKind $kind, AgreementContext $context, int $id, array $input): array
    {
        ...
        $coverageEnd = $this->coverageEnd($agreement);
    ...
    private function coverageEnd(Agreement $agreement): ?string
    {
        ...
        // Closing an agreement must stop new commercial coverage even when the original contract was
        // open-ended. Historical periods through the closure civil date remain billable; closing does
        // not rewrite already recorded contract dates or previously issued charges.
        $timezone = (string) config('app.timezone', self::DATE_TIMEZONE);
