...
use Modules\VehicleRental\Http\Requests\ActivateRentalRateVersionRequest;
...
final class RentalRateVersionController
{
    ...
    public function store(
        ...
        int $agreement,
        RentalRateVersionService $service,
    ): JsonResponse {
        ...
        return (new RentalRateVersionResource($service->createDraft(
            $agreementModel,
            $request->validated(),
    ...
    public function activate(
        ActivateRentalRateVersionRequest $request,
        int $version,
        RentalRateVersionService $service,
    ): RentalRateVersionResource {
        $this->authorization->assert($request->currentUserId(), $request->tenantId(), VehicleRentalAuthorizationService::MANAGE_RATES);
        $model = $this->scope(RentalAgreementRateVersion::query(), $request)->findOrFail($version);

        return new RentalRateVersionResource($service->activate(
            $model,
