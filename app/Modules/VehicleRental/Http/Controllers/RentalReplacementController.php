...
final class RentalReplacementController
{
    ...
    public function show(ListRentalRequest $request, int $replacement): JsonResponse
    {
        ...
        return response()->json(['data' => [
            'id' => (int) $model->getKey(), 'replacement_number' => $model->replacement_number,
            'replacement_at' => $model->replacement_at?->toISOString(), 'reason_code' => $model->reason_code,
            'reason' => $model->reason, 'billing_continuity_rule' => $model->billing_continuity_rule,
