...
final class OrganizationUnitService implements OrganizationUnitServiceInterface
{
    ...
    {
        try {
            return Result::success($this->units->listByTenant($this->domain->ensureTenantId($tenantId)));
    ...
    public function create(array $payload): Result
    {
        try {
            ...
            if ($this->units->findByTenantAndName($tenantId, $name) !== null) {
                return Result::failure(new Error(OrganizationUnitErrorCode::CONFLICT, 'Organization unit name already exists for tenant.'));
            }
            ...
            if ($typeId !== null) {
                ...
                if ($type === null || (int) $type->require('tenant_id') !== $tenantId) {
                    return Result::failure(new Error(OrganizationUnitErrorCode::TENANT_MISMATCH, 'Type must belong to same tenant.'));
                }
            }
            ...
            if ($parentId !== null) {
                ...
                if ($parent === null || (int) $parent->require('tenant_id') !== $tenantId) {
                    return Result::failure(new Error(OrganizationUnitErrorCode::TENANT_MISMATCH, 'Parent unit must belong to same tenant.'));
                }
            }
            ...
            $record = $this->units->create([
                ...
                'parent_id' => $parentId,
                'name' => $name,
                'code' => $this->domain->normalizeOptionalText(isset($payload['code']) ? (string) $payload['code'] : null, 255),
                'image_path' => $this->domain->normalizeOptionalText(isset($payload['image_path']) ? (string) $payload['image_path'] : null, 2048),
                'path' => $this->domain->normalizeOptionalText(isset($payload['path']) ? (string) $payload['path'] : null, 1024),
                ...
                'description' => $this->domain->normalizeOptionalText(isset($payload['description']) ? (string) $payload['description'] : null),
    ...
    public function update(int|string $id, array $payload): Result
    {
        try {
            ...
            if (array_key_exists('tenant_id', $payload) && (int) $payload['tenant_id'] !== $tenantId) {
                return Result::failure(new Error(OrganizationUnitErrorCode::TENANT_MISMATCH, 'Organization unit tenant cannot be changed.'));
            }
            ...
            if ($parentId !== null) {
                if ((string) $parentId === (string) $id) {
                    return Result::failure(new Error(OrganizationUnitErrorCode::CONFLICT, 'Organization unit cannot be its own parent.'));
                }
            ...
            $record = $this->units->update($id, [
                ...
                'code' => array_key_exists('code', $payload)
                    ? $this->domain->normalizeOptionalText(isset($payload['code']) ? (string) $payload['code'] : null, 255)
                    : $existing->get('code'),
                'image_path' => array_key_exists('image_path', $payload)
                    ? $this->domain->normalizeOptionalText(isset($payload['image_path']) ? (string) $payload['image_path'] : null, 2048)
                    : $existing->get('image_path'),
                'path' => array_key_exists('path', $payload)
                    ? $this->domain->normalizeOptionalText(isset($payload['path']) ? (string) $payload['path'] : null, 1024)
                    : $existing->get('path'),
                ...
                'description' => array_key_exists('description', $payload)
                    ? $this->domain->normalizeOptionalText(isset($payload['description']) ? (string) $payload['description'] : null)
                    : $existing->get('description'),
                '_lft' => array_key_exists('_lft', $payload) ? (int) $payload['_lft'] : (int) $existing->get('_lft', 0),
                '_rgt' => array_key_exists('_rgt', $payload) ? (int) $payload['_rgt'] : (int) $existing->get('_rgt', 0),
