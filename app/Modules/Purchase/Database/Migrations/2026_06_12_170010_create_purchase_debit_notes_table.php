...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('purchase_debit_notes', function (Blueprint $table): void {
            ...
            $table->foreignId('tenant_id')->constrained('tenants', indexName: 'purchase_debit_notes_tenant_fk')->restrictOnDelete();
            ...
            $table->index(['tenant_id', 'organization_unit_id'], 'purchase_debit_notes_tenant_org_ix');
            $table->index(['supplier_type', 'supplier_id'], 'purchase_debit_notes_supplier_ix');
            $table->index('purchase_return_id', 'purchase_debit_notes_return_ix');
            $table->index('status', 'purchase_debit_notes_status_ix');
            $table->index(['source_type', 'source_id'], 'purchase_debit_notes_source_ix');
            $table->index(['status', 'allocated_amount', 'remaining_amount'], 'purchase_debit_notes_allocation_ix');
