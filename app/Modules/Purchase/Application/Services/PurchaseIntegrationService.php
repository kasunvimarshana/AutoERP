...
final class PurchaseIntegrationService implements PurchaseIntegrationServiceInterface
{
    private const ENTITY_TYPES = ['purchase_order', 'grn_header', 'purchase_return'];

    public function __construct(
        private readonly PurchaseWorkflowServiceInterface $workflowService,
    ...
    public function matchSourceDocumentLine(
        ...
        try {
            ...
            if ($documentItem === null) {
                return Result::failure(new Error(
                    PurchaseErrorCode::NOT_FOUND,
                    'Document line not found for this supplier document.',
                ));
            }
    ...
    public function unmatchSourceDocumentLine(
        ...
        try {
            ...
            if (! $match instanceof DataRecord) {
                return Result::failure(new Error(PurchaseErrorCode::NOT_FOUND, 'Document line match not found.'));
            }
    ...
    public function createSourcePayment(string $entityType, int|string $id, array $payload): Result
    {
        try {
            ...
            if ($documentId < 1) {
                return Result::failure(new Error(
                    PurchaseErrorCode::INVALID_VALUE,
                    'A linked supplier document is required before creating payment.',
                ));
            }
            ...
            if (! $payment instanceof DataRecord) {
                return Result::failure(new Error(PurchaseErrorCode::INVALID_VALUE, 'Unexpected payment response.'));
            }
    ...
    public function createSourceAdvance(string $entityType, int|string $id, array $payload): Result
    {
        try {
            ...
            if ($documentId < 1) {
                return Result::failure(new Error(
                    PurchaseErrorCode::INVALID_VALUE,
                    'A linked supplier document is required before creating advance payment.',
                ));
            }
            ...
            $supplierId = (int) $record->get('supplier_id', 0);
            if ($supplierId < 1) {
                return Result::failure(new Error(
                    PurchaseErrorCode::INVALID_VALUE,
                    'supplier_id is required on source entity for advance creation.',
                ));
            }
            ...
            if (! $advance instanceof DataRecord) {
                return Result::failure(new Error(PurchaseErrorCode::INVALID_VALUE, 'Unexpected advance response.'));
            }
    ...
    public function supplierPayables(int $tenantId, ?int $supplierId): Result
    {
        try {
            ...
            foreach ($sourceRecords as $source) {
                ...
                $sourceSupplierId = (int) $source->get('supplier_id', 0);
                if ($sourceId < 1 || $sourceSupplierId < 1) {
                    continue;
                }

                $summaryResult = $this->sourcePaymentSummary('purchase_order', $sourceId, ['tenant_id' => $tenantId]);
                ...
                if (! isset($bySupplier[$sourceSupplierId])) {
                    $bySupplier[$sourceSupplierId] = [
                        'supplier_id' => $sourceSupplierId,
                        'outstanding_amount' => 0.0,
                    ];
                }

                $bySupplier[$sourceSupplierId]['outstanding_amount'] += $outstanding;
            }
    ...
    private function findEntity(string $entityType, int|string $id): ?DataRecord
    {
        return match ($entityType) {
            'purchase_order' => $this->purchaseOrderRepository->findById($id),
    ...
    private function findSourceLine(string $entityType, int $sourceLineId, int $sourceId): ?DataRecord
    {
        $line = match ($entityType) {
            'purchase_order' => $this->purchaseOrderLineRepository->findById($sourceLineId),
            'grn_header' => $this->grnLineRepository->findById($sourceLineId),
            'purchase_return' => $this->purchaseReturnLineRepository->findById($sourceLineId),
            default => null,
        };
        ...
        $isLinkedToSource = match ($entityType) {
            'purchase_order' => (int) $line->get('purchase_order_id', 0) === $sourceId,
            'grn_header' => (int) $line->get('grn_header_id', 0) === $sourceId,
            'purchase_return' => (int) $line->get('purchase_return_id', 0) === $sourceId,
            default => false,
        };
    ...
    private function resolveAvailableSourceLineQuantity(
        ...
        $baseQuantity = match ($entityType) {
            ...
            'grn_header' => (float) $sourceLine->get('accepted_qty', $sourceLine->get('received_qty', 0)),
            'purchase_return' => (float) $sourceLine->get('return_qty', 0),
            default => 0.0,
        };
