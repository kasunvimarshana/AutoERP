...
final class PlatformMfaService
{
    ...
    public function verify(int $userId, ?string $totpCode, ?string $backupCode): bool
    {
        return $this->database->transaction(function () use ($userId, $totpCode, $backupCode): bool {
            $method = AuthPlatformMfaMethodModel::query()
                ->where('user_id', $userId)
                ->where('status', PlatformMfaStatus::ACTIVE)
                ->lockForUpdate()
            ...
            if (! $verified && is_string($backupCode) && $this->normalizeBackupCode($backupCode) !== '' && is_array($remainingHashes)) {
                foreach ($remainingHashes as $index => $hash) {
                    if (is_string($hash) && $this->passwords->verify($this->normalizeBackupCode($backupCode), $hash)) {
    ...
    private function authenticateOperator(string $email, string $password, string $ipAddress, string $purpose): Result
    {
        ...
        return Result::success([
            'id' => (int) $operator->id(),
            'email' => (string) $operator->get('email', $email),
        ]);
    }
