    ...
    public function up(): void
    {
        Schema::create('auth_platform_refresh_tokens', function (Blueprint $table): void {
            ...
            $table->foreign('access_token_id', 'auth_platform_refresh_access_fk')
                ->references('id')
                ->on('auth_platform_access_tokens')
                ->cascadeOnDelete();
            $table->foreign(['platform_session_id', 'user_id'], 'auth_platform_refresh_session_user_fk')
                ->references(['id', 'user_id'])
                ->on('auth_platform_sessions')
                ->cascadeOnDelete();
            $table->unique('refresh_key', 'auth_platform_refresh_tokens_key_uk');
            $table->index(['user_id', 'status'], 'auth_platform_refresh_user_status_idx');
