...
final class VehicleCategoryService
{
    ...
    public function delete(VehicleCategory $category): void { if ($category->children()->exists()) { throw new InvalidArgumentException('Vehicle category cannot be deleted while child categories reference it.'); } if ($category->vehicles()->exists()) { throw new InvalidArgumentException('Vehicle category cannot be deleted while vehicles reference it.'); } $category->delete(); }
