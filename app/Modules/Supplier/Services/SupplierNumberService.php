...
{
    private const NUMBER_DOCUMENT_TYPE = 'supplier';

    private const CODE_DOCUMENT_TYPE = 'supplier_code';
    ...
    {
        return $this->nextUnique($tenantId, self::NUMBER_DOCUMENT_TYPE, 'supplier_number');
    }
    ...
    private function nextUnique(int $tenantId, string $documentType, string $column): string
    {
        ...
        throw new RuntimeException('Unable to generate a unique supplier reference.');
    }
    ...
    private function advanceSequencePastExistingReferences(
        ...
        if ($updated === null) {
            throw new RuntimeException('Supplier sequence was changed while resynchronizing. Try again.');
        }
    }

    private function highestExistingNumber(int $tenantId, string $column): int
    {
        $references = Supplier::query()
            ->withTrashed()
            ->where('tenant_id', $tenantId)
