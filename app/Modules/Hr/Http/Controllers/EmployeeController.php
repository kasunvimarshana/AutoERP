...
use Modules\Hr\Http\Requests\EmployeeActionRequest;
...
final class EmployeeController
{
    ...
    public function destroy(EmployeeActionRequest $request, int $employee): JsonResponse { $this->queries->delete($this->queries->employee($employee, $request->tenantId(), $request->organizationUnitId()), $request->rowVersion()); return response()->json(null, 204); }
    public function activate(EmployeeActionRequest $request, int $employee): EmployeeResource { return $this->changeTo($request, $employee, EmployeeStatus::Active); }
    public function deactivate(EmployeeActionRequest $request, int $employee): EmployeeResource { return $this->changeTo($request, $employee, EmployeeStatus::Inactive); }
    ...
    private function changeTo(EmployeeActionRequest $request, int $id, EmployeeStatus $status): EmployeeResource { return new EmployeeResource($this->statuses->changeTo($this->queries->employee($id, $request->tenantId(), $request->organizationUnitId()), $status, $request->rowVersion(), $request->currentUserId())); }
