...
final class HrEngineTest extends TestCase
{
    ...
    public function test_employee_creation_status_history_and_lookup(): void
    {
        ...
        app(EmployeeStatusService::class)->change($employee, new EmployeeStatusChangeData(EmployeeStatus::OnLeave, 'Annual leave', 1));
    ...
    public function test_rate_overlap_and_cross_tenant_reference_are_rejected(): void
    {
        ...
        $rates->create($employee, new EmployeeRateData(EmployeeRateType::Hourly, '10.000000', $currencyId, '2026-01-01', '2026-12-31'));

        try {
            $rates->create($employee, new EmployeeRateData(EmployeeRateType::Hourly, '12.000000', $currencyId, '2026-06-01', '2027-01-01'));
    ...
    public function test_one_shot_api_creates_readable_employee_graph_and_relations(): void
    {
        ...
        $response = $this->postJson('/api/v1/hr/employees/with-relations', [
            'tenant_id' => $tenantId,
        ...
        $this->getJson("/api/v1/hr/employees/lookup/service-available?tenant_id={$tenantId}&organization_unit_id={$organizationId}&skill_id={$skill->getKey()}")
            ->assertOk()->assertJsonFragment(['employee_number' => 'EMP-API-1']);
        $this->getJson("/api/v1/hr/employees/{$id}/contacts?tenant_id={$tenantId}&organization_unit_id={$organizationId}")
    ...
    public function test_master_crud_relation_crud_status_and_validation_api(): void
    {
        ...
        $department = $this->postJson('/api/v1/hr/departments', ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'code' => 'API-DEP', 'name' => 'API Department'])
            ->assertCreated()->json('data');
        $this->getJson("/api/v1/hr/departments/lookup?tenant_id={$tenantId}&organization_unit_id={$organizationId}")
            ->assertOk()->assertJsonFragment(['name' => 'API Department']);

        $employee = $this->postJson('/api/v1/hr/employees', ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'employee_number' => 'EMP-CRUD', 'first_name' => 'Amal', 'display_name' => 'Amal', 'department_id' => $department['id'], 'status' => 'pending_approval'])
            ->assertCreated()->json('data');
        $id = (int) $employee['id'];
        $contact = $this->postJson("/api/v1/hr/employees/{$id}/contacts", ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'contact_name' => 'Primary', 'is_primary' => true])
            ->assertCreated()->json('data');
        $this->putJson("/api/v1/hr/employees/{$id}/contacts/{$contact['id']}", ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'contact_name' => 'Updated Primary', 'is_primary' => true])
            ->assertOk()->assertJsonPath('data.contact_name', 'Updated Primary');
        $this->patchJson("/api/v1/hr/employees/{$id}/status", ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'status' => 'active', 'reason' => 'Approved'])
            ->assertOk()->assertJsonPath('data.status', 'active');
        $this->putJson("/api/v1/hr/employees/{$id}", ['tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, 'mobile' => '0771234567'])
            ->assertOk()->assertJsonPath('data.mobile', '0771234567');
        $this->getJson("/api/v1/hr/employees/{$id}/status-history?tenant_id={$tenantId}&organization_unit_id={$organizationId}")
            ->assertOk()->assertJsonCount(2, 'data');
        $this->postJson('/api/v1/hr/employees', ['tenant_id' => $tenantId, 'first_name' => '', 'status' => 'invalid', 'default_hourly_rate' => '-1'])
    ...
    public function test_all_master_and_assignment_endpoints(): void
    {
        ...
        foreach ([
            ...
            $masters[$endpoint] = $this->postJson("/api/v1/hr/{$endpoint}", [
        ...
        foreach ($relations as $endpoint => $payload) {
            $this->postJson("/api/v1/hr/employees/{$id}/{$endpoint}", [
                'tenant_id' => $tenantId, 'organization_unit_id' => $organizationId, ...$payload,
            ])->assertCreated();
            $this->getJson("/api/v1/hr/employees/{$id}/{$endpoint}?tenant_id={$tenantId}&organization_unit_id={$organizationId}")
                ->assertOk()->assertJsonCount(1, 'data');
        }
        $this->postJson("/api/v1/hr/employees/{$id}/availability", [
            'tenant_id' => $tenantId, 'organization_unit_id' => $organizationId,
    ...
    public function test_tenant_and_organization_isolation_on_reads(): void
    {
        ...
        $this->getJson("/api/v1/hr/employees/{$employee->getKey()}?tenant_id={$otherTenantId}&organization_unit_id={$otherOrganizationId}")
            ->assertNotFound();
        $this->getJson("/api/v1/hr/employees?tenant_id={$otherTenantId}&organization_unit_id={$otherOrganizationId}")
    ...
    {
        return app(EmployeeCreationService::class)->create(new CreateEmployeeData(tenantId: $tenantId, organizationUnitId: $organizationId, employeeNumber: $number, firstName: $number, displayName: $number, departmentId: $departmentId, status: EmployeeStatus::Active));
    }
