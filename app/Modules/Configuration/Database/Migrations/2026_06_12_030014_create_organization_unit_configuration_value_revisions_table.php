...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('organization_unit_configuration_value_revisions', function (Blueprint $table): void {
            ...
            $table->unique(
                ['id', 'tenant_id'],
                'organization_unit_configuration_revisions_id_tenant_uk',
            );
            $table->unique(
                ['id', 'tenant_id', 'organization_unit_id'],
                'organization_unit_configuration_revisions_identity_uk',
            );
            $table->foreign(
                ['source_revision_id', 'tenant_id', 'organization_unit_id'],
                'organization_unit_configuration_revisions_source_scope_fk',
            )->references(['id', 'tenant_id', 'organization_unit_id'])
                ->on('organization_unit_configuration_value_revisions')
                ->restrictOnDelete();
            $table->foreign(
                ['organization_unit_id', 'tenant_id'],
                'organization_unit_configuration_revisions_org_tenant_fk',
    ...
    {
        Schema::dropIfExists('organization_unit_configuration_value_revisions');
    }
};
