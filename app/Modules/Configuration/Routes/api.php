...
$centralHost = (string) config(
    'tenant.platform.central_host_middleware_alias',
...
Route::prefix('api/v1/platform/defaults')
    ->middleware(['api', $centralHost, 'auth:'.$guard, $currentUser, $currentTenant])
    ->name('api.v1.platform.defaults.')
    ->group(function () use ($registerScope): void {
        Route::get('definitions', [ConfigurationController::class, 'definitions'])
            ->defaults('scope', ConfigurationScope::GLOBAL)
            ->name('definitions');
        $registerScope('', ConfigurationScope::GLOBAL);
    });
