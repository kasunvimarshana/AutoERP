...
final class SalesInvoiceController extends Controller
{
    ...
    {
        try {
            return (new SalesOrderResource($this->lifecycle->createInvoice($request->all())))->response()->setStatusCode(201);
        } catch (Throwable $exception) {
            return response()->json(['message' => $exception->getMessage()], 422);
        }
    }

    public function approve(int|string $id): JsonResponse|SalesOrderResource
    {
        try {
            return new SalesOrderResource($this->lifecycle->approveInvoice((int) $id));
        } catch (Throwable $exception) {
            return response()->json(['message' => $exception->getMessage()], 422);
        }
    }
