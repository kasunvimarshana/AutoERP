...
final class SalesInvoiceIntegrationService
{
    ...
    public function createCustomerInvoice(CreateSalesInvoiceData $data): Invoice
    {
        return DB::transaction(function () use ($data): Invoice {
            ...
            foreach ($normalized['lineQuantities'] as $key => $quantity) {
                [$type, $id] = explode(':', $key, 2);
                if ($type === 'sales_delivery_line') {
                    $line = SalesDeliveryLine::query()->with('salesOrderLine')->findOrFail((int) $id);
                    $line->invoiced_quantity = $this->math->add((string) $line->invoiced_quantity, $quantity);
                    $line->remaining_quantity = $this->math->sub((string) $line->delivered_quantity, (string) $line->invoiced_quantity);
                    $line->status = $this->math->isZero((string) $line->remaining_quantity) ? 'invoiced' : 'partially_invoiced';
                    $line->save();
                    if ($line->salesOrderLine instanceof SalesOrderLine) {
                        $this->orders->applyInvoiced($line->salesOrderLine, $quantity);
                    }
                } elseif ($type === 'sales_order_line') {
                    $this->orders->applyInvoiced(SalesOrderLine::query()->findOrFail((int) $id), $quantity);
                }
            }

            $this->refreshDeliveryStatuses($normalized['deliveries']);
    ...
    {
        return $this->invoices->preview($this->normalize($data)['invoiceData']);
    }

    /**
     * @return array{
     *   invoiceData: CreateInvoiceData,
     *   sourceTotals: array<string, array{line_total: string, adjustment_total: string}>,
     *   lineQuantities: array<string, string>,
     *   deliveries: Collection<int, SalesDelivery>
     * }
     */
    private function normalize(CreateSalesInvoiceData $data): array
    {
        if ($data->sources === []) {
        ...
        foreach ($data->sources as $source) {
            ...
            if ($source->sourceType === 'sales_delivery') {
                $delivery = SalesDelivery::query()->with(['lines.salesOrderLine', 'adjustments'])->findOrFail($source->sourceId);
            ...
            $order = SalesOrder::query()->with(['lines', 'adjustments'])->findOrFail($source->sourceId);
