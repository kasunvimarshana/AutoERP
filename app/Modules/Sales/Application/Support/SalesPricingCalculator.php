...
final class SalesPricingCalculator
{
    public function __construct(
        private readonly ?UomConversionResolverServiceInterface $uomResolver = null,
    ...
    public function normalizeOrderPayload(array $payload): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $line = $this->applyUomNormalization($line, $payload, 'ordered_qty');
    ...
    public function normalizeInvoicePayload(array $payload): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $unitPrice = $this->decimal($line['unit_price'] ?? '0.000000');
            $line = $this->applyUomNormalization($line, $payload, 'quantity');
     ...
     * @return array<string, mixed>
     */
    private function applyUomNormalization(array $line, array $payload, string $quantityField): array
    {
        if ($this->uomResolver === null) {
            return $line;
        }

        $tenantId = isset($payload['tenant_id']) ? (int) $payload['tenant_id'] : null;
        $productId = isset($line['product_id']) ? (int) $line['product_id'] : null;
        $fromUomId = isset($line['uom_id']) ? (int) $line['uom_id'] : null;
        $quantity = $this->decimal($line[$quantityField] ?? '0.000000');

        if ($tenantId === null || $productId === null || $fromUomId === null || bccomp($quantity, '0.000000', 6) === 0) {
            return $line;
        }

        try {
            $normalized = $this->uomResolver->normalizeToProductBase($tenantId, $productId, $fromUomId, $quantity, 6);
            $line['base_uom_id'] = $normalized['base_uom_id'];
            $line['base_quantity'] = $normalized['quantity'];
            $line['uom_conversion_factor'] = $normalized['factor'];
        } catch (\Throwable $exception) {
            throw new \InvalidArgumentException(
                sprintf(
                    'UOM conversion is not resolvable for product_id %d from uom_id %d: %s',
                    $productId,
                    $fromUomId,
