...
final class TaxEngineTest extends TestCase
{
    ...
    public function test_tax_creation_rates_groups_and_validation_guardrails(): void
    {
        ...
        $tax = $this->withTenantExecutionContext(
            $tenantId,
            fn (): Tax => $service->saveTax($this->taxPayload($tenantId, 'GEN-15', 'General Tax', 'VAT', 'exclusive', payable: true)),
        );
        $this->assertSame('GEN-15', $tax->code);

        $this->withTenantExecutionContext(
            $tenantId,
            fn () => $service->saveRate($tax, [
                'rate' => '15.000000',
    ...
    public function test_tax_groups_reject_duplicate_sequence_inactive_tax_and_duplicate_code(): void
    {
        ...
        $group = $this->withTenantExecutionContext(
            $tenantId,
            fn (): TaxGroup => $service->saveGroup([
                'tenant_id' => $tenantId,
                'code' => 'DEFAULT',
                'name' => 'Default Taxes',
                'is_default' => true,
    ...
    public function test_tax_determination_uses_item_profile_document_and_default_priority(): void
    {
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxMasterDataService::class)->saveCustomerProfile([
                'tenant_id' => $tenantId,
        ...
        $itemResult = $this->withTenantExecutionContext(
            $tenantId,
            fn () => $determine->determine(new TaxDeterminationContext(
                tenantId: $tenantId,
        ...
        $profileResult = $this->withTenantExecutionContext(
            $tenantId,
            fn () => $determine->determine(new TaxDeterminationContext(
                tenantId: $tenantId,
                documentType: 'sales_invoice',
        ...
        $zeroRated = $this->withTenantExecutionContext(
            $tenantId,
            fn () => $determine->determine(new TaxDeterminationContext(
                tenantId: $tenantId,
                documentType: 'sales_invoice',
    ...
    public function test_header_tax_uses_configured_group_without_line_tax_duplication(): void
    {
        ...
        $result = $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxCalculationService::class)->calculate(new TaxCalculationData(
                tenantId: $tenantId,
                documentType: 'sales_invoice',
                documentDate: '2026-06-11',
    ...
    public function test_posting_profile_resolution_generates_finance_posting_context(): void
    {
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxMasterDataService::class)->savePostingProfile([
                'tenant_id' => $tenantId,
                'tax_id' => $tax->getKey(),
                'direction' => 'output',
                'posting_key' => 'tax_payable',
                'active' => true,
            ]),
        );

        $context = $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxPostingContextService::class)->build(
                ...
                counterpartyProfileKey: 'receivable',
                counterpartyLineName: 'Accounts Receivable',
            ),
        );
    ...
    public function test_tax_reports_snapshots_transactions_tenant_and_organization_isolation(): void
    {
        ...
        $this->withTenantExecutionContext($tenantId, function () use ($tenantId, $orgId, $group): void {
        ...
        $this->withTenantExecutionContext($otherTenant, function () use ($otherTenant): void {
        ...
        $summary = $this->withTenantExecutionContext(
            $tenantId,
            fn (): array => app(TaxReportService::class)->summary($tenantId, $orgId, ['date_from' => '2026-06-01', 'date_to' => '2026-06-30']),
        );
        $this->assertSame('100.000000', $summary['totals']['taxable_amount']);
        $this->assertSame('10.000000', $summary['totals']['tax_amount']);

        $reconciliation = $this->withTenantExecutionContext(
            $tenantId,
            fn (): array => app(TaxReportService::class)->reconciliation($tenantId, $orgId),
        );
        $this->assertSame('0.000000', $reconciliation['totals']['difference']);

        $isolated = $this->withTenantExecutionContext(
            $tenantId,
            fn (): array => app(TaxReportService::class)->summary($tenantId, null),
        );
    ...
    public function test_invoice_integration_creates_snapshots_and_posts_transactions_once(): void
    {
        ...
        $this->withTenantExecutionContext($tenantId, function () use ($invoiceId): void {
    ...
    public function test_posted_purchase_snapshot_is_immutable_after_rate_change(): void
    {
        ...
        $this->withTenantExecutionContext($tenantId, function () use ($grn): void {
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxDocumentIntegrationService::class)->snapshot(app(GoodsReceiptNoteTaxDocumentMapper::class)->map($grn->refresh())),
        );
    }
    ...
    public function test_posted_invoice_snapshot_is_immutable_after_rate_change(): void
    {
        ...
        $this->withTenantExecutionContext($tenantId, function () use ($invoice): void {
            $integration = app(TaxDocumentIntegrationService::class);
        ...
        $this->assertSame(
            ...
            $this->withTenantExecutionContext(
                $tenantId,
                fn (): string => (string) TaxDocumentSnapshot::query()
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxDocumentIntegrationService::class)->snapshot(app(InvoiceTaxDocumentMapper::class)->map($invoice->refresh())),
        );
    }
    ...
    public function test_partial_purchase_return_reverses_original_snapshot_tax_and_blocks_duplicate(): void
    {
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxDocumentIntegrationService::class)->post(app(GoodsReceiptNoteTaxDocumentMapper::class)->map($grn)),
        );
        $sourceLineId = $this->withTenantExecutionContext(
            $tenantId,
            fn (): int => (int) $grn->lines()->firstOrFail()->getKey(),
        );
        $return = $this->createPurchaseReturn($tenantId, $supplierId, $warehouseId, $sourceLineId, $itemId, '2.000000', '10.000000', '100.000000');
        $created = $this->withTenantExecutionContext(
            $tenantId,
            fn (): array => app(TaxReturnAllocationService::class)->reverse(app(PurchaseReturnTaxDocumentMapper::class)->map($return)),
        );
        ...
        $summary = $this->withTenantExecutionContext(
            $tenantId,
            fn (): array => app(TaxReportService::class)->summary($tenantId, null, ['tax_code' => 'PIN-10']),
        );
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => app(TaxReturnAllocationService::class)->reverse(app(PurchaseReturnTaxDocumentMapper::class)->map($return->refresh())),
        );
    }
    ...
    public function test_wht_invoice_context_generates_finance_context_and_missing_profile_fails(): void
    {
        ...
        $this->withTenantExecutionContext(
            $tenantId,
            fn () => $integration->snapshot(app(InvoiceTaxDocumentMapper::class)->map($invoice)),
        );
        $this->assertSame(
            '5000.000000',
            $this->withTenantExecutionContext(
                $tenantId,
                fn (): string => (string) TaxDocumentSnapshot::query()->where('source_type', 'invoice')->where('is_withholding', true)->value('tax_amount'),
            ),
        );
        ...
        try {
            $this->withTenantExecutionContext(
                $tenantId,
                fn () => $integration->withholdingPostingContextForDocument(app(InvoiceTaxDocumentMapper::class)->map($invoice->refresh()), '2026-06-11', 'purchase_invoice', 'payable', 'Supplier payable'),
            );
            $this->fail('Expected missing WHT posting profile to fail.');
        } catch (InvalidArgumentException $exception) {
            $this->assertStringContainsString('Tax posting profile is missing', $exception->getMessage());
        }
        ...
        $this->withTenantExecutionContext(
            ...
            fn () => app(TaxMasterDataService::class)->savePostingProfile([
                ...
                'posting_key' => 'withholding_payable',
                'active' => true,
            ]),
        );
        ...
        $this->assertSame('purchase_invoice', $context->financeContext->postingProfileCode);
        $this->assertSame('payable', $context->financeContext->lines[0]->profileKey);
        $this->assertSame('withholding_payable', $context->financeContext->lines[1]->profileKey);
    }
    ...
    public function test_wht_payment_context_uses_allocated_invoice_snapshot_proportion(): void
    {
        ...
        $context = $this->withTenantExecutionContext(
            $tenantId,
            fn () => $integration->withholdingPostingContextForPayment(app(PaymentTaxWithholdingMapper::class)->map($payment->refresh()), '2026-06-11', 'payment_made', 'payable', 'Supplier payable'),
        );

        $this->assertSame('payment', $context->source->sourceType);
        $this->assertSame('2000.000000', $context->taxLines[0]->taxAmount);
        $this->assertSame('payment_made', $context->financeContext->postingProfileCode);
        $this->assertSame('withholding_payable', $context->financeContext->lines[1]->profileKey);
    }
    ...
    private function createTax(
        ...
        return $this->withTenantExecutionContext($tenantId, function () use ($tenantId, $code, $type, $method, $rate, $organizationUnitId, $isWithholding, $service): Tax {
    ...
    private function createGroup(int $tenantId, string $code, Tax $tax, bool $isDefault = false, ?int $organizationUnitId = null): TaxGroup
    {
        return $this->withTenantExecutionContext(
            $tenantId,
            fn (): TaxGroup => app(TaxMasterDataService::class)->saveGroup([
                'tenant_id' => $tenantId,
    ...
    private function createGoodsReceiptNote(
        ...
        return $this->withTenantExecutionContext(
            $tenantId,
            fn (): GoodsReceiptNote => GoodsReceiptNote::query()->with('lines')->findOrFail($grnId),
        );
    }
    ...
    private function createInvoice(
        ...
        return $this->withTenantExecutionContext(
            $tenantId,
            fn (): Invoice => Invoice::query()->with('lines')->findOrFail($invoiceId),
        );
    }
    ...
    private function createPayment(int $tenantId, Invoice $invoice, string $allocatedAmount): Payment
    {
        ...
        DB::table('payment_allocations')->insert([
            ...
            'invoice_number_snapshot' => (string) $invoice->invoice_number,
            'invoice_date_snapshot' => $invoice->invoice_date?->toDateString(),
            'invoice_currency_code_snapshot' => null,
