...
export function CompanySettingsPage() {
    ...
    const settingsQuery = useTenantSettings(
        selectedTenantId,
        { group: 'company', per_page: 100, is_public: parseBooleanSearchParam(visibility) },
        selectedTenantId > 0,
    );
    ...
    return (
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
            <PageHeader
                ...
                description="Company settings now read from tenant-scoped settings records instead of the generic placeholder route."
                title="Company Settings"
            />

            <ContentCard className="p-0">
                <TableToolbar description="Inspect tenant-level company settings, presentation values, and any public/private split already stored in the backend." title="Company setting registry">
                    {tenantsQuery.isPending ? (
                        ...
                        <EmptyState description="A tenant is required before company settings can be displayed." title="No tenants available" />
                    ) : (
                        <SearchFilterToolbar
                            filters={
                                <>
                ...
                {!activeTenant ? (
                    <EmptyState className="m-6" description="Select a tenant to inspect company settings." title="No tenant selected" />
                    ...
                    isForbiddenError(settingsQuery.error) ? (
                        ...
                        <ErrorState className="m-6" description={settingsQuery.error.message} title="Unable to load company settings" />
                    )
                ) : (
                    <DataTable
                        columns={columns}
                        emptyState={<EmptyState className="m-6" description="No company settings match the current filters for this tenant." title="No company settings found" />}
