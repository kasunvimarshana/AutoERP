...
export const supplierFormSchema = z.object({
    supplier_code: optionalTrimmedString(255, 'Supplier code must be 255 characters or less.'),
    name: z.string().trim().min(1, 'Supplier name is required.').max(255, 'Supplier name must be 255 characters or less.'),
    type: z.enum(['individual', 'company'], { message: 'Choose a valid supplier type.' }),
    ...
    ap_account_id: optionalInteger('Choose a valid AP account.'),
    status: z.enum(['active', 'inactive'], { message: 'Choose a valid supplier status.' }),
