...
export function OrganizationUnitLegalProfilePanel({
    ...
    const [form, setForm] = useState<LegalProfileForm>(() => emptyForm(unit.name));
    const [saving, setSaving] = useState(false);
    const [actionError, setActionError] = useState<ApiError | null>(null);

    useEffect(() => {
        setForm(profile.data ? profileForm(profile.data) : emptyForm(unit.name));
    }, [profile.data, unit.name]);
    ...
    const save = async (event: FormEvent) => {
        ...
        try {
            const payload: OrganizationUnitLegalProfilePayload = {
                ...(profile.data ? { expected_version: profile.data.row_version } : {}),
    ...
    return (
        <Panel>
            ...
            <ErrorAlert error={actionError ?? profile.error} />
