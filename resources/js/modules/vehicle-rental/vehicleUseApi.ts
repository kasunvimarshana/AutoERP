...
export const planVehicleUse = (agreement: Agreement, input: { vehicle_id: number; owner_agreement_id: number | null; starts_at: string; ends_at: string; notes: string | null }) => apiClient.post<ApiResource<VehicleUse>>(agreementUses(agreement.id), { ...input, expected_version: agreement.row_version }).then(r => r.data.data);
