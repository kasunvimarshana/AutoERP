...
export function VehicleUseEditor({ agreement, replacement, onSaved, onCancel }: { agreement: Agreement; replacement?: VehicleUse; onSaved: () => void; onCancel: () => void }) {
    ...
    return <form onSubmit={submit} className="space-y-4 rounded-lg border p-4" aria-label={replacement ? "Replace vehicle" : "Assign vehicle"}>
        ...
        <fieldset disabled={saving} className="grid gap-4 sm:grid-cols-2">
            ...
            <Input label={replacement ? "Replacement time" : "Planned handover"} type="datetime-local" step={OPERATIONAL_TIME_STEP_SECONDS} value={start} onChange={e => setStart(e.target.value)} required error={error?.fields.starts_at?.[0]} />
            <Input label="Planned return (optional)" type="datetime-local" step={OPERATIONAL_TIME_STEP_SECONDS} value={end} onChange={e => setEnd(e.target.value)} error={error?.fields.ends_at?.[0]} />
