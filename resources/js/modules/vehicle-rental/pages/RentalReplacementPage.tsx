...
type AllocationLookupValue = VersionedNamedResource & {
...
function ownershipCoversPeriod(
    ownership: PartyVehicleRelationship,
    ...
    return (
        startedAt.getTime() <= periodStartsAt.getTime() &&
        (endedAt === null || endedAt.getTime() >= periodEndsAt.getTime())
    );
}

function ownershipLabel(
    ownership: PartyVehicleRelationship | null,
): string {
...
export default function RentalReplacementPage() {
    ...
    const [sourceAllocation, setSourceAllocation] =
        useState<AllocationLookupValue | null>(null);
    const [financeAgreement, setFinanceAgreement] =
        useState<VersionedNamedResource | null>(null);
    const [companyOwnership, setCompanyOwnership] =
        useState<PartyVehicleRelationship | null>(null);
    const [companyOwnershipLoading, setCompanyOwnershipLoading] =
        useState(false);
    const [companyOwnershipHint, setCompanyOwnershipHint] = useState<
        string | null
    >(null);
            ...
            .then((response) => {
                ...
                const ownership =
                    response.data.find(
                        (row) =>
                            row.is_current &&
                            ownershipCoversPeriod(
                                row,
                                selectedStartAt,
                                selectedEndAt,
                            ),
                    ) ??
                    response.data.find((row) =>
                        ownershipCoversPeriod(
                            row,
                            selectedStartAt,
                            selectedEndAt,
    ...
    const submit = async (event: FormEvent) => {
        ...
        if (!allocationId || !vehicle || !allocation.data || !agreementVersion) {
            return;
        }
        if (
            form.vehicle_source_type === SOURCE_TYPE_COMPANY_OWNED &&
        ...
        setActionError(null);
        try {
            await replaceRentalVehicle(
                allocationId,
                allocation.data.row_version,
                agreementVersion,
                {
                    ...
                    source_allocation_id:
                        form.vehicle_source_type === SOURCE_TYPE_OWNER_SUPPLIED
                    ...
                    old_return: {
                        ...oldReturn,
                        fuel_level_percent:
                            oldReturn.fuel_level_percent || null,
                    },
                    new_handover: {
                        ...newHandover,
                        fuel_level_percent:
                            newHandover.fuel_level_percent || null,
                    },
                },
            );
            ...
            <RentalPage>
                ...
                <Panel>
                    <p className="text-sm text-slate-600">
                        Open the replacement workflow from a valid rental allocation.
                    </p>
                </Panel>
            </RentalPage>
        );
    ...
    const replacementEnd =
        form.allocated_to ||
        toDateTimeLocal(
            allocation.data.agreement?.ends_at ?? allocation.data.allocated_to,
        );
    const replacementStartAt = form.replacement_at
        ? toIsoDateTime(form.replacement_at)
        : null;
    const replacementEndAt = replacementEnd
        ? toIsoDateTime(replacementEnd)
        : null;
    const agreementVersion = allocation.data.agreement?.row_version;
    const allocationStart = toDateTimeLocal(allocation.data.allocated_from);
    const allocationEnd = toDateTimeLocal(
        allocation.data.agreement?.ends_at ?? allocation.data.allocated_to,
    );
    const usesOwnerSourceAllocation =
        form.vehicle_source_type === SOURCE_TYPE_OWNER_SUPPLIED;
    ...
    return (
        <RentalPage>
            <ContentHeader
                ...
                description="Close the old allocation, record its return and hand over the replacement as one atomic transaction. Existing agreement billing periods remain unchanged until a replacement billing policy is approved."
            />
            <ErrorAlert
                error={actionError ?? allocation.error ?? metadata.error}
            />
                    ...
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <Input
                                ...
                                value={
                                    companyOwnershipLoading
                                        ? "Checking ownership..."
                                        : ownershipLabel(companyOwnership)
                                }
                                readOnly
                                required
                        ...
                        {usesOwnerSourceAllocation && (
                            <RentalAllocationLookupSelect
                                ...
                                openOnly
                                disabled={
                                    !replacementStartAt || !replacementEndAt
                                }
                        ...
                        {form.vehicle_source_type === SOURCE_TYPE_FINANCED && (
                            <RentalFinanceAgreementLookupSelect
                                ...
                                onChange={(value) =>
                                    setFinanceAgreement(
                                        value as VersionedNamedResource | null,
                                    )
                                }
