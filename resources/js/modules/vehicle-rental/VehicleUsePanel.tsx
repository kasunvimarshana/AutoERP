...
export function VehicleUsePanel({ agreement, canManage }: { agreement: Agreement; canManage: boolean }) {
    ...
    function reload() { setLoading(true); setAdding(false); setAction(null); setHistory(null); setRevision(v => v + 1); }
    ...
    return <section aria-label="Assigned vehicles" className="space-y-4 border-t pt-4">
        <div className="flex gap-2"><h3 className="grow text-lg font-semibold">Assigned vehicles</h3><Button variant="secondary" disabled={saving || adding} onClick={reload}>Reload vehicles</Button>{canManage && agreement.status === AgreementStatus.Active && <Button disabled={saving || adding || action !== null} onClick={() => setAdding(true)}>Assign vehicle</Button>}</div>
        <ErrorAlert error={error} inline />
        {adding && <VehicleUseEditor agreement={agreement} onSaved={reload} onCancel={() => setAdding(false)} />}
        {loading ? <p role="status">Loading vehicle use…</p> : rows.map(row => <article key={row.id} className="space-y-2 rounded-lg border p-4">
            <p className="font-medium">{row.vehicle.label} · {USE_LABELS[row.status]}</p><p>{row.starts_at} — {row.ends_at}</p><p>{row.owner_agreement ? `Owner: ${row.owner_agreement.party_name} · ${row.owner_agreement.reference}` : 'Company supply'}</p>
            ...
            <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={saving} onClick={() => setHistory(history === row.id ? null : row.id)}>Vehicle-use history</Button>{canManage && !adding && !action && row.status === VehicleUseStatus.Planned && <><Button onClick={() => choose(row, VehicleUseAction.Handover)}>Hand over vehicle</Button><Button variant="secondary" onClick={() => choose(row, VehicleUseAction.Cancel)}>Cancel plan</Button></>}{canManage && !adding && !action && row.status === VehicleUseStatus.InCustody && <Button onClick={() => choose(row, VehicleUseAction.Return)}>Record return</Button>}</div>
