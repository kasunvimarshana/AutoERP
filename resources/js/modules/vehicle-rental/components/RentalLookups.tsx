...
export function RentalAllocationLookupSelect({ agreementId, ...props }: LookupProps & { agreementId?: number | null }) {
    const search = useCallback(
        (params: LookupLoadParams) => searchAllocations(params, agreementId),
        [agreementId],
    );
...
export function RentalFinanceAgreementLookupSelect(props: LookupProps) {
    return <LookupSelect label="Finance agreement" search={searchFinanceAgreements} placeholder="Search finance agreement..." {...props} />;
}
...
async function searchFinanceAgreements(
    ...
    const response = await listVehicleFinanceAgreements({ search, page, per_page: perPage }, signal);
