...
export function VehicleForm({ initial, submitting, error, enableRelations = false, onSubmit }: {
    ...
    const [ownershipDraft, setOwnershipDraft] = useState<VehicleOwnershipPayload>({ owner_type: 'customer', owner_id: null, ownership_type: 'customer_owned', started_at: businessDateInputValue(), is_current: true });
                ...
                <TabPanel tabsId="vehicle-form" tabId="ownership" active={tab}>
                    <RelationDraft disabled={!enableRelations} emptyMessage="Ownerships are saved from the detail page when editing.">
                        <div className="grid gap-3 md:grid-cols-3">
                            <Select label="Owner Type" value={ownershipDraft.owner_type} options={ownerTypes.map((value) => ({ value, label: value.replaceAll('_', ' ') }))} onChange={(event) => {
                            ...
                            {ownershipDraft.owner_type !== 'company' && (
                                <LookupSelect label={ownershipDraft.owner_type === 'customer' ? 'Customer' : 'Supplier'} value={ownershipOwner} onChange={setOwnershipOwner} search={ownershipDraft.owner_type === 'customer' ? lookupApi.customers : lookupApi.suppliers} error={fieldError(error, 'ownerships.0.owner_id')} />
                            )}
                        </div>
                        <Button type="button" onClick={() => {
                            setOwnerships([...ownerships, {
                                ...ownershipDraft,
                                owner_id: ownershipDraft.owner_type === 'company' ? null : ownershipOwner?.id ?? null,
                            }]);
                            setOwnershipDraft({ owner_type: 'customer', owner_id: null, ownership_type: 'customer_owned', started_at: businessDateInputValue(), is_current: true });
                            setOwnershipOwner(null);
