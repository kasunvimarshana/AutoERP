...
export default function GoodsReceiptDetailPage() {
    ...
    const post = async () => {
        if (!await confirm({
            ...
            message: 'Confirm posting for this goods receipt?',
            confirmLabel: 'Post',
        })) return;
        ...
        setActionError(null);
        try {
            result.setData(await postGoodsReceipt(grn.id, { expected_version: grn.row_version }));
        } catch (requestError) {
    ...
    const reverse = async (facts: ReversalFacts) => {
        ...
        setActionError(null);
        try {
            result.setData(await reverseGoodsReceipt(grn.id, {
                expected_version: grn.row_version,
                ...facts,
            }));
            ...
            <PurchaseDocumentShell
                header={<PurchasePageHeader
                    ...
                    actions={<div className="flex flex-wrap justify-end gap-2">
                        ...
                        {capabilities.can_post && can(purchasePermissions.goodsReceiptsPost) && <Button loading={busy} onClick={() => void post()}>Post</Button>}
                        {capabilities.can_reverse && can(purchasePermissions.goodsReceiptsReverse) && <Button disabled={busy} variant="secondary" onClick={() => setReversalOpen(true)}>Reverse</Button>}
