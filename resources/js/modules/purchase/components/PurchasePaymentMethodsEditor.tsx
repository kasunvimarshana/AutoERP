...
export function paymentRowSatisfiesMethod(row: PurchasePaymentMethodRow, method?: FastPurchaseOptionResource): boolean {
    ...
    const hasInstrumentDetails = Boolean(
        row.instrument_number.trim()
        || row.instrument_date
        || row.external_bank_name.trim()
        || row.external_bank_branch.trim(),
    );
                    ...
                    return (
                        <article key={row.key} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                            <div className="grid gap-3">
                                ...
                                />
                                <Input
                                    label={method?.requires_reference ? 'Reference *' : 'Reference'}
                                    value={row.reference}
                                ...
                                {method?.requires_instrument_details && !paymentRowSatisfiesMethod(row, method) && (
                                    <p className="text-sm text-amber-700">This method requires transaction instrument details.</p>
                                )}
                                <DecimalInput
                                    label="Amount"
                                    value={row.amount}
                ...
                <table className="min-w-full table-fixed text-sm">
                    <thead className="bg-slate-50 text-left text-xs font-semibold uppercase text-slate-500">
                        <tr>
                            ...
                            <th className="px-3 py-2">Reference / instrument details</th>
                        ...
                        {rows.map((row, index) => {
                            ...
                            return (
                                <tr key={row.key} className="align-top">
                                    ...
                                    <td className="px-3 py-3">
                                        <Input value={row.reference} placeholder={method?.requires_reference ? 'Reference required' : 'Reference'} onChange={(event) => update(index, { reference: event.target.value })} error={errorFor(`lines.${errorIndex}.reference`) ?? errorFor(`payment.lines.${errorIndex}.reference`)} />
                                        <MethodSpecificFields methodType={method?.method_type} row={row} index={index} errorIndex={errorIndex} errorFor={errorFor} update={update} />
                                        {method?.requires_instrument_details && !paymentRowSatisfiesMethod(row, method) && (
                                            <p className="mt-2 text-xs text-amber-700">Instrument details required.</p>
                                        )}
                                    </td>
    ...
    if (type === 'cheque') {
        return (
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
                ...
                <Input placeholder="External bank" value={row.external_bank_name} onChange={(event) => update(index, { external_bank_name: event.target.value })} error={errorFor(`lines.${errorIndex}.external_bank_name`) ?? errorFor(`payment.lines.${errorIndex}.external_bank_name`)} />
                <Input placeholder="External branch" value={row.external_bank_branch} onChange={(event) => update(index, { external_bank_branch: event.target.value })} error={errorFor(`lines.${errorIndex}.external_bank_branch`) ?? errorFor(`payment.lines.${errorIndex}.external_bank_branch`)} />
            </div>
        );
    }
