            ...
            {adjustments.map((adjustment, index) => (
                <div key={index} className="rounded-lg border border-slate-200 p-4">
                    ...
                    <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        {adjustment.calculation_type === 'percentage' && <Select label="Base" value={adjustment.calculation_base} options={calculationBases} error={errorFor(`adjustments.${index}.calculation_base`)} onChange={(event) => update(index, { ...adjustment, calculation_base: event.target.value as EditableHeaderAdjustment['calculation_base'] })} />}
                        {adjustment.calculation_type === 'percentage'
                            ? <Input label="Rate" type="number" min="0" max="100" step="0.000001" value={adjustment.rate} error={errorFor(`adjustments.${index}.rate`)} onChange={(event) => update(index, { ...adjustment, rate: event.target.value })} />
