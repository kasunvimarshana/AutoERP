...
const emptyPaymentMethod: Partial<PaymentMethod> = {
    code: '',
    name: '',
    method_type: 'cash',
...
export default function PaymentMethodFormPage() {
    ...
    const existing = useApi((signal) => id !== null ? getPaymentMethod(id, signal) : Promise.resolve(null), [id], id !== null);

    if (id !== null && existing.loading) return <LoadingState />;

    return (
        <PaymentMethodEditor
            key={id ?? 'create'}
            id={id}
            initialValue={existing.data}
            loadError={existing.error}
        />
    );
}

function PaymentMethodEditor({ id, initialValue, loadError }: {
    id: number | null;
    initialValue: PaymentMethod | null;
    loadError: ApiError | null;
}) {
    const navigate = useNavigate();
    const [form, setForm] = useState<Partial<PaymentMethod>>(() => initialValue ? { ...initialValue } : { ...emptyPaymentMethod });
    ...
    async function submit(event: FormEvent) {
        ...
        try {
            ...
            if (id !== null) await updatePaymentMethod(id, payload);
    ...
    return (
        <>
            <ContentHeader
                title={id !== null ? 'Edit Payment Method' : 'Create Payment Method'}
            ...
            <Panel>
                <form className="max-w-3xl space-y-5" onSubmit={(event) => void submit(event)}>
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button type="submit" loading={busy}>{id !== null ? 'Save Method' : 'Create Method'}</Button>
                    </div>
                </form>
            </Panel>
        </>
