...
export default function PlatformOperatorsPage() {
    ...
    const columns = useMemo<DataColumn<PlatformOperator>[]>(() => [
        {
            key: 'operator',
            header: 'Operator',
            render: (operator) => (
            ...
            render: (operator) => canManage ? (
                <div className="flex justify-end gap-2">
                    ...
                    <Button
                        ...
                        className="min-h-8 px-3 py-1 text-xs"
                        onClick={() => {
                            setAction({ type: 'status', operator, target: operator.status === 'active' ? 'inactive' : 'active' });
                            setReason('');
                            setError(null);
                        }}
                    >
    ...
    async function createOperator(event: FormEvent) {
        ...
        try {
            ...
            request.reload();
            setSuccess(`${created.display_name} can now sign in to the platform control plane and complete MFA enrollment.`);
    ...
    return (
        <>
            <ContentHeader
                ...
                description="Govern control-plane identities, least-privilege grants, and operator lifecycle without mixing tenant users into platform access."
            ...
            <div className="space-y-5">
                ...
                {request.loading && !request.data ? <LoadingState label="Loading platform operators..." /> : (
                    <DataTable
                        ...
                        mobileActions={canManage ? (operator) => (
                            <div className="flex gap-2">
                                <Button variant="secondary" onClick={() => openPermissions(operator)}>Permissions</Button>
                                <Button
                                    variant={operator.status === 'active' ? 'danger' : 'primary'}
                                    onClick={() => setAction({ type: 'status', operator, target: operator.status === 'active' ? 'inactive' : 'active' })}
                                >
                                    {operator.status === 'active' ? 'Deactivate' : 'Activate'}
                                </Button>
                            </div>
            ...
            <Modal open={createOpen} title="Create platform operator" onClose={() => setCreateOpen(false)} closeDisabled={saving}>
                <form className="space-y-5" onSubmit={createOperator}>
                    <p className="rounded-lg bg-blue-50 p-3 text-sm text-blue-900">
                        This creates a dedicated control-plane identity. The operator must enroll MFA before normal platform access.
                    </p>
                    ...
                    <Input label="Platform email" type="email" required value={createForm.email} onChange={(event) => setCreateForm({ ...createForm, email: event.target.value })} />
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Input label="Initial password" type="password" required value={createForm.password} onChange={(event) => setCreateForm({ ...createForm, password: event.target.value })} />
                        <Input label="Confirm password" type="password" required value={createForm.password_confirmation} onChange={(event) => setCreateForm({ ...createForm, password_confirmation: event.target.value })} />
                    </div>
