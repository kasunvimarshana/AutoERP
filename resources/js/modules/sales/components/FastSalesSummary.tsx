...
interface FastSalesSummaryProps {
    preview: FastSalesResult | null;
    result: FastSalesResult | null;
    submitting: boolean;
    previewing: boolean;
    canSubmit: boolean;
    onPreview: () => void;
...
export function FastSalesSummary({ preview, result, submitting, previewing, canSubmit, onPreview }: FastSalesSummaryProps) {
    ...
    const links = [
        ...
        documents?.goods_delivery,
        documents?.customer_invoice,
        documents?.customer_receipt,
    ...
    return (
        <aside className="sticky bottom-0 top-4 space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:w-80">
            ...
            <Button type="submit" className="w-full" loading={submitting} disabled={!canSubmit}>Create fast sale</Button>
            {links.length > 0 && (
