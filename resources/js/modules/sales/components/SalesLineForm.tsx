...
import { getSalesItemContext } from '../salesApi';
import type { SalesItemContext } from '../salesTypes';
import { SalesItemLookupSelect } from './SalesLookups';
import {
    previewLineAmounts,
    type EditableSalesLine,
    type SalesLineCalculationType,
...
export function SalesLineForm({ line, mode, config, currencyId, warehouseId, salesDate, errorFor, onSave, onCancel }: {
    ...
    warehouseId?: number;
    salesDate?: string;
    errorFor: (field: SalesLineField) => string | undefined;
    onSave: (line: EditableSalesLine) => void;
    onCancel: () => void;
}) {
    const [draft, setDraft] = useState(line);
    const [context, setContext] = useState<SalesItemContext | null>(null);
    ...
    const set = <K extends keyof EditableSalesLine>(key: K, value: EditableSalesLine[K]) => {
        ...
        void getSalesItemContext(draft.item.id, {
            ...
            .then((nextContext) => {
                ...
                setDraft((current) => {
                    ...
                    const resolvedUomId = nextContext.uom_id ?? nextContext.default_sales_uom_id;
                    const defaultUom = nextContext.allowed_sales_uoms.find((row) => row.id === resolvedUomId);
    ...
    }, [config.taxMode, draft.item?.id, draft.item_variant_id, draft.uom?.id, currencyId, warehouseId, salesDate, contextReload]);
        ...
        <form ref={formRef} className="space-y-5" onSubmit={(event) => {
            ...
            <section className="space-y-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Item, quantity, UOM, and sales price are enough for most lines.</p>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <Select
                        ...
                        onChange={(event) => {
                            ...
                            const selected = context?.allowed_sales_uoms.find((row) => row.id === uomId)?.uom ?? null;
                            setDraft((current) => ({
                                ...current,
                                uom: selected,
                                auto_uom: false,
...
function CalculationField({ label, type, rate, amount, typeError, valueError, onTypeChange, onValueChange }: {
    ...
    typeError?: string;
    valueError?: string;
    onTypeChange: (type: SalesLineCalculationType) => void;
    ...
    return (
        <>
            <Select
                ...
                error={typeError}
                onChange={(event) => onTypeChange(event.target.value as SalesLineCalculationType)}
            />
            <DecimalInput
...
function validateLineForm(line: EditableSalesLine, config: SalesLineEditorConfig): LineFormErrors {
...
function fieldToBackendField(field: keyof LineFormErrors): SalesLineField {
