...
export default function TaxListPage() {
    ...
    const columns: DataColumn<Tax>[] = [
        { key: 'tax', header: 'Tax', render: (row) => <Link className="font-semibold text-sky-700 hover:underline" to={`/tax/taxes/${row.id}/edit`}>{row.code}<span className="block text-xs font-normal text-slate-500">{row.name}</span></Link> },
    ...
    return (
        <>
            <ContentHeader title="Tax Engine" description="Configurable tax master data, rates, and calculation methods." actions={<div className="flex gap-3 text-sm font-semibold"><Link className="text-sky-700 hover:underline" to="/tax/groups">Tax groups</Link><Link className="rounded-lg bg-sky-600 px-4 py-2 text-white hover:bg-sky-700" to="/tax/taxes/create">New tax</Link></div>} />
