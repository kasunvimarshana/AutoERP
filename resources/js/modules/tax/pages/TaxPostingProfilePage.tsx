...
export default function TaxPostingProfilePage() {
    ...
    const columns: DataColumn<TaxPostingProfile>[] = [
        ...
        { key: 'edit', header: '', className: 'text-right', render: (row) => <button type="button" className="text-sm font-semibold text-sky-700" onClick={() => { setSelectedId(row.id); setForm({ tax_id: String(row.tax_id), direction: row.direction, posting_key: row.posting_key, active: row.active }); }}>Edit</button> },
    ];
