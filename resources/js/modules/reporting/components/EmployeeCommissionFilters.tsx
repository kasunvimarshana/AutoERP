        ...
        <form className="space-y-4" onSubmit={onApply}>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                <Input
                    ...
                    placeholder="Employee name, number, or job"
                />
            ...
            <div>
                <Button type="button" variant="secondary" onClick={() => setShowMore((current) => !current)}>
                    {showMore ? 'Hide more filters' : 'More filters'}
                </Button>
            </div>
            {showMore && <div className="grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4 md:grid-cols-2 xl:grid-cols-4">
                <Select
                    label="Group by"
