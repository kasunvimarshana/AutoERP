...
export function ConfigurationSettingsPanel({ permissions, hasOrganizationUnit, mode = 'tenant' }: Props) {
    const scopes = useMemo(() => availableScopes(mode, hasOrganizationUnit), [mode, hasOrganizationUnit]);
    ...
    const definitions = useApi((signal) => listConfigurationDefinitions(mode, signal), [mode]);
    const entries = useApi(
        (signal) => listConfigurationEntries(scope, prefix, page, signal),
        [scope, prefix, page],
    );
    const canManage = scope === 'global'
        ? permissions.includes(settingsPermissions.platformDefaultsManage)
        : scope === 'organization_unit'
            ? permissions.includes(settingsPermissions.manageOrganization)
            : permissions.includes(settingsPermissions.manageTenant);
    const canManageSensitive = scope === 'global'
        ? permissions.includes(settingsPermissions.platformDefaultsManageSensitive)
        : permissions.includes(settingsPermissions.manageSensitive);
    const historyDefinitions = (definitions.data ?? []).filter((definition) =>
    ...
    const columns: DataColumn<ConfigurationEntry>[] = [
        ...
        { key: 'updated', header: 'Updated', render: (row) => new Date(row.updated_at).toLocaleString() },
        {
            ...
            header: '',
            render: (row) => {
                const canMutateEntry = canManage && (!row.sensitive || canManageSensitive);

                return (
                    <div className="flex justify-end gap-2">
                        <Button variant="secondary" onClick={() => setHistoryKey(row.key)}>History</Button>
                        {canMutateEntry && <>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="grid gap-4 lg:grid-cols-[minmax(220px,0.7fr)_minmax(260px,1fr)_auto] lg:items-end">
                    <Select
                        ...
                        options={scopes}
                        onChange={(event) => {
                            setScope(event.target.value as ConfigurationScope);
                            setPage(1);
                            setHistoryKey(null);
                        }}
                    />
                    ...
                    <div className="flex flex-wrap gap-2">
                        {historyDefinitions.length > 0 && (
                            <Button
                                variant="secondary"
                                onClick={() => setHistoryKey(historyDefinitions[0]?.key ?? null)}
                            >
                                Browse history
                            </Button>
                        )}
                ...
                <p className="mt-3 text-sm text-slate-500">
                    {mode === 'platform'
                        ? 'Platform defaults affect tenants that do not define a narrower override. Every change is recorded.'
                        : 'Only registered settings can be changed. Removing an override restores the next inherited value.'}
                </p>
            </div>
...
function ConfigurationForm({ scope, definitions, existing, entry, submitting, canManageSensitive, error, onCancel, onSubmit }: {
    ...
    const [rawValue, setRawValue] = useState(() => initialEditorValue(entry, definition));
...
function availableScopes(
    ...
    if (mode === 'platform') {
        return [{ value: 'global', label: 'Platform defaults' }];
    }
...
function initialEditorValue(
    entry: ConfigurationEntry | null,
    definition: ConfigurationDefinition | null,
): string {
    if (entry?.sensitive) return '';

    return toEditorValue(entry?.value ?? definition?.default_value);
}
