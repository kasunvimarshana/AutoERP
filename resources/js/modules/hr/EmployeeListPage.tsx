...
export default function EmployeeListPage() {
    ...
    const toggle = (row: EmployeeSummary) => setEmployeeActive(row.id, row.status !== 'active', row.row_version).then((employee) => setRows((current) => current.map((x) => x.id === row.id ? { ...x, status: employee.status, availability_status: employee.availability_status, row_version: employee.row_version } : x))).catch((e) => setError(toApiError(e)));
    return <div><ContentHeader title="Employees" description="Workforce profiles, qualifications, rates, and assignment availability." actions={canCreate ? <LinkButton to="/hr/employees/create">Create Employee</LinkButton> : undefined} />
