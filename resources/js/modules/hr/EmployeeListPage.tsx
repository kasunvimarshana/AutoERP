...
export default function EmployeeListPage() {
    ...
    return <div><ContentHeader title="Employees" description="Workforce profiles, qualifications, rates, and assignment availability." actions={<Link to="/hr/employees/create"><Button>Create Employee</Button></Link>} />
        ...
        <ErrorAlert error={error} />{loading ? <LoadingState label="Loading employees..." /> : <><DataTable rows={rows} rowKey={(row) => row.id} columns={[
            ...
            { key: 'actions', header: '', render: (row) => <div className="flex justify-end gap-2"><Link to={`/hr/employees/${row.id}`}><Button variant="ghost">View</Button></Link><Link to={`/hr/employees/${row.id}/edit`}><Button variant="ghost">Edit</Button></Link>{row.status !== 'terminated' && <Button variant="secondary" onClick={() => void toggle(row)}>{row.status === 'active' ? 'Deactivate' : 'Activate'}</Button>}</div> },
