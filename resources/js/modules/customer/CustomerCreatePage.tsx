...
export default function CustomerCreatePage() {
    ...
    return <><ContentHeader title="New customer" description="Create a basic customer or save an initial customer relation tree transactionally." /><div className="mb-4 flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4"><input id="customer-one-shot" type="checkbox" checked={oneShot} onChange={(event) => { setOneShot(event.target.checked); setActiveTab('basic'); }} /><label htmlFor="customer-one-shot" className="text-sm font-medium">One-shot relation-aware create</label></div><ErrorAlert error={error} /><form className="space-y-5" onSubmit={(event) => { event.preventDefault(); void save(); }}>{oneShot && <Panel className="p-0"><Tabs tabs={tabs} active={activeTab} onChange={setActiveTab} /></Panel>}{(!oneShot || activeTab === 'basic') && <CustomerForm value={customer} onChange={setCustomer} currency={currency} onCurrencyChange={setCurrency} error={error} creating />}{oneShot && activeTab !== 'basic' && <Panel><CustomerOneShotBuilder section={activeTab} value={draft} onChange={setDraft} /></Panel>}<div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => navigate(-1)}>Cancel</Button><Button type="submit" loading={submitting}>{oneShot ? 'Create customer tree' : 'Create customer'}</Button></div></form></>;
}
