...
export default function SupplierCreatePage() {
    ...
    return <><ContentHeader title="New supplier" description="Create a basic supplier or save an initial supplier relation tree transactionally." /><div className="mb-4 flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4"><input id="supplier-one-shot" type="checkbox" checked={oneShot} onChange={(event) => { setOneShot(event.target.checked); setActiveTab('basic'); }} /><label htmlFor="supplier-one-shot" className="text-sm font-medium">One-shot relation-aware create</label></div><ErrorAlert error={error} /><form className="space-y-5" onSubmit={(event) => { event.preventDefault(); void save(); }}>{oneShot && <Panel className="p-0"><Tabs tabs={tabs} active={activeTab} onChange={setActiveTab} /></Panel>}{(!oneShot || activeTab === 'basic') && <SupplierForm value={supplier} onChange={setSupplier} currency={currency} onCurrencyChange={setCurrency} error={error} creating />}{oneShot && activeTab !== 'basic' && <Panel><SupplierOneShotBuilder section={activeTab} value={draft} onChange={setDraft} /></Panel>}<div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => navigate(-1)}>Cancel</Button><Button type="submit" loading={submitting}>{oneShot ? 'Create supplier tree' : 'Create supplier'}</Button></div></form></>;
}
