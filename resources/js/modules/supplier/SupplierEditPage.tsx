...
export default function SupplierEditPage() {
    ...
    async function save() {
        ...
        try { const saved = await updateSupplier(supplierId, form); navigate(`/suppliers/${saved.id}`); }
        catch (requestError) { setError(toApiError(requestError)); }
        finally { setSubmitting(false); }
    }
    return <><ContentHeader title="Edit supplier" description="Basic supplier profile only; owned relations remain on-demand in detail tabs." /><ErrorAlert error={error} /><form className="space-y-5" onSubmit={(event) => { event.preventDefault(); void save(); }}><SupplierForm value={form} onChange={setForm} currency={currency} onCurrencyChange={setCurrency} error={error} /><div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => navigate(-1)}>Cancel</Button><Button type="submit" loading={submitting}>Save supplier</Button></div></form></>;
}
