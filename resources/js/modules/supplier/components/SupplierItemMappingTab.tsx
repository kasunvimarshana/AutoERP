...
export default function SupplierItemMappingTab({ supplierId, canManage }: { supplierId: number; canManage: boolean }) {
    ...
    const columns: DataColumn<SupplierItemMapping>[] = [
        ...
        ...(canManage ? [{ key: 'actions', header: '', className: 'text-right', render: (row: SupplierItemMapping) => <Actions edit={() => crud.startEdit(row)} remove={() => crud.destroy(row)} /> }] : []),
    ];
    return <><SupplierRelationHeader title="Item mappings" description="Supplier-specific item codes, names, UOMs, and ordering references." onAdd={canManage ? crud.startCreate : undefined} /><ErrorAlert error={crud.actionError ?? crud.error} />{crud.loading ? <LoadingState /> : <DataTable rows={crud.data?.data ?? []} columns={columns} rowKey={(row) => row.id} mobileSummary={(row) => row.supplier_item_code ?? row.supplier_item_name ?? row.item?.name ?? '-'} mobileDetails={(row) => <div className="grid grid-cols-2 gap-2 text-sm"><span>Internal: {row.item?.name ?? '-'}</span><span>UOM: {row.default_purchase_uom?.code ?? '-'}</span></div>} />}<Pagination meta={crud.data?.meta} onPageChange={crud.setPage} />{canManage && <FormDrawer open={crud.open} title={crud.editing ? 'Edit item mapping' : 'Add item mapping'} onClose={crud.close}>{crud.open && <MappingForm key={crud.editing?.id ?? 'new'} row={crud.editing} error={crud.actionError} submitting={crud.submitting} onCancel={crud.close} onSubmit={crud.submit} />}</FormDrawer>}{canManage && crud.confirmDialog}</>;
}
