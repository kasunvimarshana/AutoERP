...
export default function PlatformTenantsPage() {
    ...
    const readiness = useApi(
        (signal) => getTenantReadiness(selected?.id ?? 0, signal),
    ...
    return (
        <>
            ...
            <div className="space-y-5">
                <ErrorAlert error={tenants.error ?? plans.error ?? currencies.error ?? readiness.error ?? error} />
                        ...
                        <div className="space-y-3">
                            ...
                            {canLifecycle && selected.status !== 'archived' && <>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {['draft','inactive','suspended'].includes(selected.status) && <Button loading={busyAction === 'activate'} disabled={!reason.trim() || !readiness.data?.ready_for_activation} onClick={() => void transition('activate')}>Activate</Button>}
...
function TenantReadinessPanel({ readiness, loading, onRefresh }: {
    readiness: import('./tenantTypes').TenantReadiness | null;
    loading: boolean;
    onRefresh: () => void;
}) {
    if (loading && !readiness) {
        return <div className="rounded-lg border border-slate-200 p-3 text-sm text-slate-500">Checking activation readiness...</div>;
    }
        ...
        <div className="rounded-lg border border-slate-200 p-3">
            <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-slate-900">
                    {readiness.ready_for_activation ? 'Ready for activation' : 'Activation requirements'}
                </p>
                <Button
                    variant="secondary"
                    loading={loading}
                ...
                {readiness.checks.map((check) => (
                    <li key={check.key} className="flex gap-2 text-sm">
                        ...
                        <span>
                            ...
                            {!check.ready && <span className="block text-xs text-slate-500">{check.guidance}</span>}
                        </span>
                    </li>
                ))}
            </ul>
