...
describe('invoice lifecycle handoff', () => {
    ...
    it('keeps invoices from retired source modules read-only', () => {
        ...
        expect(detail).toContain("const retiredSourceInvoiceTypes = new Set(['rental', 'vehicle_finance'])");
        expect(detail).toContain('!isRetiredSourceInvoice && value.status');
        expect(detail).toContain('This historical invoice is read-only because its source module has been retired.');
    });
});
