...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
    ...
    return (
        <div className="block text-sm text-slate-700">
            {label ? (
                <label className="mb-1.5 block font-medium" htmlFor={inputId}>
                    {label}
                </label>
            ) : null}
            <input
            ...
            />
            {error ? (
                <span id={messageId} className="mt-1 block text-xs text-rose-600">
                    {error}
                </span>
            ) : hint ? (
                <span id={messageId} className="mt-1 block text-xs text-slate-500">
                    {hint}
                </span>
            ) : null}
        </div>
