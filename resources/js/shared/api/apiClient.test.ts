...
describe('public API request classification', () => {
    it('recognizes public authentication endpoints', () => {
    ...
    it('normalizes absolute and query-bearing URLs before classification', () => {
        ...
        expect(isPublicApiRequest('https://example.test/api/v1/platform/operators?x=1')).toBe(false);
    });
});

describe('shouldAttachAuthorizationHeader', () => {
    it('never sends a stored bearer token to public login endpoints', () => {
        expect(shouldAttachAuthorizationHeader('/api/v1/auth/login', 'tenant-token')).toBe(false);
        expect(shouldAttachAuthorizationHeader('/api/v1/platform/auth/login', 'platform-token')).toBe(false);
    });
