...
import { getAgreementById, getRunningChartById } from '../mock/vehicleRentalMock';
import { vehicleRentalApi } from '../services/vehicleRentalApi';
import type { VehicleRentalRunningChart } from '../types/vehicleRental.types';
...
export function RunningChartCreatePage() {
    return (
        <div className="space-y-6">
            <VehicleRentalPageHeader
                actions={<><Link to="/vehicle-rental/running-charts"><Button variant="secondary">Cancel</Button></Link><Button variant="blue">Preview Billing</Button></>}
...
export function RunningChartDetailPage() {
    const { id = 'rc-001' } = useParams();
    const [activeTab, setActiveTab] = useState('overview');
    const [chart, setChart] = useState<VehicleRentalRunningChart>(getRunningChartById(id));
    const agreement = getAgreementById(chart.agreementNumber);

    useEffect(() => {
        vehicleRentalApi.runningCharts.get(id).then((response) => setChart(response.data));
    }, [id]);

    return (
        <div className="space-y-6">
            <VehicleRentalPageHeader
                actions={<><Link to={`/vehicle-rental/running-charts/${chart.id}/edit`}><Button>Edit</Button></Link><Button variant="blue">Finalize</Button></>}
            ...
            {activeTab === 'provider' ? <PreviewPanel rows={[{ label: 'Provider payable', value: chart.providerPayablePreview }, { label: 'AP impact', value: agreement.financePreview.calculated.apImpact }]} title="Provider Payable Preview" /> : null}
