...
export const purchaseApi = {
    ...
    async createOrder(input: PurchaseOrderInput) { const response = await httpClient<ApiResponse<RecordAny>>('/api/purchase/purchase-orders', { body: { expected_date: input.expectedDate || null, lines: input.lines.map((line) => ({ ...linePayload(line, 'order'), ordered_qty: line.receivedQty || '1' })), notes: input.notes || null, order_date: input.orderDate, po_number: input.poNumber || null, reference: input.reference || null, supplier_id: input.supplierId, warehouse_id: input.warehouseId }, method: 'POST' }); return mapOrder(response.data); },
    async updateOrder(id: number, input: PurchaseOrderInput) { const response = await httpClient<ApiResponse<RecordAny>>(`/api/purchase/purchase-orders/${id}`, { body: { expected_date: input.expectedDate || null, lines: input.lines.map((line) => ({ ...linePayload(line, 'order'), ordered_qty: line.receivedQty || '1' })), notes: input.notes || null, order_date: input.orderDate, po_number: input.poNumber || null, reference: input.reference || null, supplier_id: input.supplierId, warehouse_id: input.warehouseId }, method: 'PUT' }); return mapOrder(response.data); },
    ...
    async createGrn(input: GrnInput) { const response = await httpClient<ApiResponse<RecordAny>>('/api/purchase/grns', { body: { grn_number: input.grnNumber || null, lines: input.lines.map((line) => linePayload(line, 'grn')), notes: input.notes || null, purchase_order_id: input.purchaseOrderId || null, received_date: input.receivedDate, reference: input.reference || null, supplier_id: input.supplierId || null, warehouse_id: input.warehouseId || null }, method: 'POST' }); return mapGrn(response.data); },
    async updateGrn(id: number, input: GrnInput) { const response = await httpClient<ApiResponse<RecordAny>>(`/api/purchase/grns/${id}`, { body: { grn_number: input.grnNumber || null, lines: input.lines.map((line) => linePayload(line, 'grn')), notes: input.notes || null, purchase_order_id: input.purchaseOrderId || null, received_date: input.receivedDate, reference: input.reference || null, supplier_id: input.supplierId || null, warehouse_id: input.warehouseId || null }, method: 'PUT' }); return mapGrn(response.data); },
    ...
    async invoiceGrn(id: number) { const response = await httpClient<ApiResponse<any>>(`/api/purchase/grns/${id}/invoice`, { method: 'POST' }); return response.data as Invoice; },
    ...
    async createReturn(input: PurchaseReturnInput) { const response = await httpClient<ApiResponse<RecordAny>>('/api/purchase/purchase-returns', { body: { lines: input.lines.map((line) => linePayload(line, 'return')), notes: input.notes || null, original_grn_id: input.originalGrnId, original_invoice_id: input.originalInvoiceId || null, reference: input.reference || null, return_date: input.returnDate, return_number: input.returnNumber || null, return_reason: input.returnReason || null }, method: 'POST' }); return mapReturn(response.data); },
    async updateReturn(id: number, input: PurchaseReturnInput) { const response = await httpClient<ApiResponse<RecordAny>>(`/api/purchase/purchase-returns/${id}`, { body: { lines: input.lines.map((line) => linePayload(line, 'return')), notes: input.notes || null, original_grn_id: input.originalGrnId, original_invoice_id: input.originalInvoiceId || null, reference: input.reference || null, return_date: input.returnDate, return_number: input.returnNumber || null, return_reason: input.returnReason || null }, method: 'PUT' }); return mapReturn(response.data); },
    ...
    async payInvoice(invoiceId: number, input: { amount: string; paymentDate: string; paymentMethodId: number; reference?: string }) { const response = await httpClient<ApiResponse<any>>(`/api/purchase/invoices/${invoiceId}/payments`, { body: { amount: input.amount, payment_date: input.paymentDate, payment_method_id: input.paymentMethodId, reference: input.reference || null }, method: 'POST' }); return response.data as Payment; },
};
