...
export const purchaseApi = {
    ...
    async createGrn(input: GrnInput) { const response = await httpClient<ApiResponse<RecordAny>>('/api/purchase/grns', { body: { grn_number: input.grnNumber || null, lines: input.lines.map((line) => linePayload(line, 'grn')), notes: input.notes || null, purchase_order_id: input.purchaseOrderId, received_date: input.receivedDate, reference: input.reference || null, supplier_id: input.supplierId || null, warehouse_id: input.warehouseId || null }, method: 'POST' }); return mapGrn(response.data); },
    async updateGrn(id: number, input: GrnInput) { const response = await httpClient<ApiResponse<RecordAny>>(`/api/purchase/grns/${id}`, { body: { grn_number: input.grnNumber || null, lines: input.lines.map((line) => linePayload(line, 'grn')), notes: input.notes || null, purchase_order_id: input.purchaseOrderId, received_date: input.receivedDate, reference: input.reference || null, supplier_id: input.supplierId || null, warehouse_id: input.warehouseId || null }, method: 'PUT' }); return mapGrn(response.data); },
