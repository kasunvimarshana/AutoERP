...
function linePayload(line: any, mode: 'grn' | 'order' | 'return') {
    return {
        ...
        ordered_qty: mode === 'order' ? line.orderedQty || line.quantity || undefined : undefined,
    };
}
...
export const purchaseApi = {
    ...
    async lookup(type: string, query: { grnId?: number; purchaseOrderId?: number; search?: string; warehouseId?: number } = {}): Promise<PurchaseLookup[]> { const response = await httpClient<ApiResponse<PurchaseLookup[]>>(`/api/purchase/lookups/${type}`, { query: { grn_id: query.grnId, purchase_order_id: query.purchaseOrderId, search: query.search, warehouse_id: query.warehouseId } }); return response.data; },
    ...
    async createOrder(input: PurchaseOrderInput) { const response = await httpClient<ApiResponse<RecordAny>>('/api/purchase/purchase-orders', { body: { ...headerPayload(input), expected_date: input.expectedDate || null, lines: input.lines.map((line) => ({ ...linePayload(line, 'order'), ordered_qty: line.orderedQty || '1' })), notes: input.notes || null, order_date: input.orderDate, po_number: input.poNumber || null, reference: input.reference || null, supplier_id: input.supplierId, warehouse_id: input.warehouseId }, method: 'POST' }); return mapOrder(response.data); },
    async updateOrder(id: number, input: PurchaseOrderInput) { const response = await httpClient<ApiResponse<RecordAny>>(`/api/purchase/purchase-orders/${id}`, { body: { ...headerPayload(input), expected_date: input.expectedDate || null, lines: input.lines.map((line) => ({ ...linePayload(line, 'order'), ordered_qty: line.orderedQty || '1' })), notes: input.notes || null, order_date: input.orderDate, po_number: input.poNumber || null, reference: input.reference || null, supplier_id: input.supplierId, warehouse_id: input.warehouseId }, method: 'PUT' }); return mapOrder(response.data); },
