...
export function PurchaseInvoiceSourcePanel({ entityId, entityType }: { entityId?: string; entityType?: 'grn_header' | 'purchase_order' | 'purchase_return' }) {
    const [message, setMessage] = useState('');

    async function generateInvoice(): Promise<void> {
        if (!entityId || !entityType) {
            setMessage('Invoice generation requires a persisted purchase source.');
            return;
        }

        await purchaseApi.previews.invoice(entityType, entityId);
        setMessage('Invoice generation requested from backend.');
    }

    return (
        <PreviewPanel
            rows={[
                { label: 'Invoice source', value: entityType && entityId ? `${entityType} ${entityId}` : 'No persisted source selected' },
                { label: 'Invoice number', value: 'Generated by the Invoice backend' },
                { label: 'Rendering', value: 'Invoice module backend' },
            ]}
            status="Invoice"
            title="Supplier Invoice"
        >
            <div className="flex flex-wrap items-center gap-2">
                <Button disabled={!entityId || !entityType} onClick={() => void generateInvoice()} type="button" variant="secondary">Generate Invoice</Button>
                {message ? <span className="text-sm text-slate-600">{message}</span> : null}
            </div>
        </PreviewPanel>
    );
...
export function PurchasePaymentAllocationPanel({ allocations = [], preview }: { allocations?: PurchasePaymentAllocation[]; preview?: Record<string, unknown> }) {
    return (
        ...
        >
            {allocations.length ? <SimpleTable columns={[['sourceReference', 'Source'], ['allocatedAmount', 'Allocated'], ['balanceAfterAllocation', 'Balance After'], ['status', 'Status']]} rows={allocations} /> : null}
        </PreviewPanel>
    );
}
    ...
    return (
        <form className="space-y-5" onSubmit={(event) => void handleSubmit(event)}>
            <FormSection description="Returnable quantity, stock reversal, AP adjustment, and refund eligibility are backend-owned." title="Purchase Return">
                <div className="grid gap-4 md:grid-cols-3">
                    <Field error={errors.supplier_id} label="Supplier"><LookupSelect disabled={isLoading} onChange={(value) => setField('supplierId', value)} options={lookups.suppliers} placeholder="Select supplier" value={values.supplierId} /></Field>
                    <Field label="Source type"><Select onChange={(event) => setField('sourceType', event.target.value)} value={values.sourceType ?? ''}><option value="">Direct return</option><option value="purchase_order">Purchase order</option><option value="grn_header">GRN</option><option value="purchase_invoice">Supplier invoice</option></Select></Field>
...
export function PurchaseSettingsForm({ settings }: { settings: PurchaseSettings }) {
    ...
    return (
        <form className="space-y-5">
            <FormSection description="Settings guide backend workflow behavior; global configuration stays outside Purchase." title="Accounting, Invoice, and Warehouse Defaults">
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <Field label="Invoice type"><Input defaultValue={settings.invoiceTypeDefinition} /></Field>
                    <Field label="Invoice matching rule"><Input defaultValue={settings.invoiceMatchingRule} /></Field>
                </div>
            </FormSection>
            <FormSection description="Sequences are previewed/generated by backend Sequence and Invoice services." title="Sequences and Workflow Rules">
                <div className="grid gap-4 md:grid-cols-3">
