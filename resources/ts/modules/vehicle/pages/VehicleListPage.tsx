        ...
        <div className="space-y-5">
            <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <h1 className="mt-1 text-3xl font-bold text-slate-950">Vehicles</h1>
                    <p className="mt-1 text-sm text-slate-500">Tenant-scoped vehicle records with paginated search and local delete updates.</p>
                </div>
                <Link className="inline-flex h-10 items-center justify-center rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700" to="/vehicles/new">
                    Create vehicle
                </Link>
            </header>
            ...
            <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_180px]">
                ...
                <select className="h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm" value={status} onChange={(event) => { setPage(1); setStatus(event.target.value as VehicleStatus | ''); }}>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                {loading ? (
                    <div className="flex items-center justify-center p-12 text-sm font-semibold text-slate-500"><Spinner /><span className="ml-3">Loading vehicles</span></div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {pageData.items.map((vehicle) => (
                                    <tr className="hover:bg-slate-50/70" key={vehicle.id}>
                                        ...
                                        <td className="px-4 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${vehicle.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{vehicle.status}</span></td>
                    ...
                    <div className="p-12 text-center"><p className="font-semibold text-slate-800">No vehicles found</p><p className="mt-1 text-sm text-slate-500">Adjust the filters or create the first vehicle.</p></div>
                )}
            </div>
