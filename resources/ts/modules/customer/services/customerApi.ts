...
function address(record: CustomerRecord['address']): PartyAddress | null {
    return record
        ? {
...
function listItem(record: CustomerRecord): PartyListItem {
    return {
...
function detail(record: CustomerRecord): PartyDetail {
    return {
        ...listItem(record),
...
export const customerApi: PartyApi = {
    async create(input) {
        const response = await httpClient<ApiResponse<CustomerRecord>>('/api/customer/customers', {
            body: payload(input),
            method: 'POST',
        });
    ...
    async get(id) {
        const response = await httpClient<ApiResponse<CustomerRecord>>(`/api/customer/customers/${id}`);

        return detail(response.data);
    },
    async list(query) {
        const response = await httpClient<ApiCollectionResponse<CustomerRecord>>('/api/customer/customers', {
            query: {
                page: query.page,
    ...
    async remove(id) {
        await httpClient<void>(`/api/customer/customers/${id}`, { method: 'DELETE' });
    },
