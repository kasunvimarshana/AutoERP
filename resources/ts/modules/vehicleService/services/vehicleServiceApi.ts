...
export const vehicleServiceApi = {
    ...
    async lookup(type: string, query?: string | { jobCardId?: number; search?: string }) { const normalized = typeof query === 'string' ? { search: query } : (query ?? {}); const response = await httpClient<ApiResponse<Lookup[]>>(`/api/vehicle-service/lookups/${type}`, { query: { job_card_id: normalized.jobCardId, limit: 100, search: normalized.search } }); return response.data; },
