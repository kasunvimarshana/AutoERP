                    ...
                    {sortOptions.length > 0 ? (
                        <div className="w-full md:w-48">
                            ...
                            >
                                ...
                                {sortOptions.map((option) => <option key={`sort-${option.value}-${option.label}`} value={option.value}>{option.label}</option>)}
                            </Select>
                        </div>
                    ) : null}
                </div>
        ...
        return (
            <div className="flex gap-2">
                ...
                >
                    ...
                    {savedViews.map((view) => <option key={`saved-view-${view.id}`} value={view.id}>{view.name}</option>)}
                </Select>
    ...
    if (filter.type === 'select' || filter.type === 'status' || filter.type === 'entity') {
        return (
            ...
            >
                ...
                {filter.options?.map((option) => <option key={`${inputId}-${option.value}-${option.label}`} value={option.value}>{option.label}</option>)}
            </Select>
        );
    }
