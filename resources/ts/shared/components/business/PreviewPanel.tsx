...
export function PreviewPanel({ children, rows, status = 'Live', subtitle, title }: PreviewPanelProps) {
    return (
        <Card className="p-5">
            <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                    <h2 className="text-sm font-bold uppercase tracking-wide text-slate-700">{title}</h2>
                    <p className="mt-1 text-sm text-slate-500">{subtitle ?? 'Backend-backed summary data.'}</p>
                </div>
