...
const variants: Record<ButtonVariant, string> = {
    ...
    primary: 'bg-black text-white shadow-sm hover:bg-slate-800',
...
export function Button({ children, className, icon, type = 'button', variant = 'primary', ...props }: ButtonProps) {
    return (
        <button
            className={cn(
                'inline-flex h-10 items-center justify-center gap-2 rounded-lg px-4 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-slate-300 disabled:cursor-not-allowed disabled:opacity-50',
                variants[variant],
                className,
            )}
            type={type}
