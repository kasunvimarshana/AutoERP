...
const fetchCustomers = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || err.message || 'Failed to load customers.'
  } finally {
    loading.value = false
  }
}
...
const createCustomer = () => {
  router.push({ name: 'sales-customer-create' })
}

const viewCustomer = (id: number) => {
  router.push({ name: 'sales-customer-detail', params: { id } })
}

const editCustomer = (id: number) => {
  router.push({ name: 'sales-customer-edit', params: { id } })
}

const confirmDelete = async (customer: Customer) => {
  if (!confirm(`Are you sure you want to delete customer "${customer.customer_name}"? This action cannot be undone.`)) {
    return
  }
