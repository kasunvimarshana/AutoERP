    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-600">
          Manage your sales orders and shipments
        </p>
      </div>
      <button
        class="btn-primary"
...
import { salesApi, type SalesOrder, type Customer } from '@/api/sales'
...
const fetchOrders = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || err.message || 'Failed to load sales orders.'
  } finally {
    loading.value = false
  }
}
...
const createOrder = () => {
  router.push({ name: 'sales-order-create' })
}

const viewOrder = (id: number) => {
  router.push({ name: 'sales-order-detail', params: { id } })
}
